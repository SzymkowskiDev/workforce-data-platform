...
root_path = os.path.join(os.path.dirname(os.path.abspath(
    __file__)), '..', '..', '..', 'research', 'sourced_data')
db_path = os.path.join(root_path, 'document_db_protocol.json')
...
def create_csv_from_document_db(db):
    ...
    skills_binary_df = pd.DataFrame(
        0, index=data.index, columns=list(unique_skills))
    ...
    data[['type', 'salary_from', 'salary_to', 'currency']] = data['employment_types'].apply(
        extract_employment_type_values).apply(pd.Series)

    data = data.drop(columns=['employment_types'])

    def extract_city_names(multilocation):
        city_names = [loc.get('city', None)
                      for loc in multilocation if isinstance(loc, dict)]
    ...
    data[['city_1', 'city_2', 'city_3', 'city_4', 'city_5']
         ] = data['multilocation'].apply(extract_city_names).apply(pd.Series)

    data = data.drop(columns=['multilocation'])

    data.to_csv(os.path.join(os.path.dirname(__file__),
                'job_offers_protocol.csv'), index=False)
